...
export function ContactForm({ contact }: { contact: Contact }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4 px-6 py-4">
      <Field name="name"  label="Имя"      required defaultValue={contact.name}    errors={errors.name} />
      <Field name="email" label="Email"                type="email" defaultValue={contact.email ?? ''} errors={errors.email} />
      <Field name="phone" label="Телефон"                            defaultValue={contact.phone ?? ''} errors={errors.phone} />
      <Field name="role"  label="Должность"                           defaultValue={contact.role ?? ''}  errors={errors.role} />
