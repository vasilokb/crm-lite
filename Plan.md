...
| 9. Воронка сделок: прогресс-бар + правила won/lost | [x] | [x] | 2026-07-05 — updateOpportunityStage полная реализация: won без amount/contact → отказ; lost без reasonLost → отказ; status↔stage синхронизированы (won/lost↔open) + closeDate; reasonLost сохраняется только при lost; StageProgressBar 5 шагов ↔ список, превентивные UX-проверки; CreateOpportunityForm на /opportunities; tsc exit 0; boundary-тесты pass |
| 10. Активности: timeline + Optimistic UI для `done` | [x] | [x] | 2026-07-05 — TaskCheckbox с useOptimistic (React 19) + useTransition; ActivityTimeline: note/task визуально различимы (✎/✓), overdue подсвечены красным + «просрочено»; ActivityForm с toggle note/task (для task dueDate обязателен); Zod-toggle pass; tsc exit 0 |
...
- **Drawer-модель** — `Drawer` рендерится через React Portal в `document.body` (иначе `<div>` Drawer нельзя вложить в `<tr>`, hydration error). Паттерн: 4 client-component `*RowWithDrawer` (`<tr onClick={() => setOpen(true)}>` + `<Link onClick={e.preventDefault()}>`) для таблиц; `CardOverlayWrapper` (client) для full-page версий (`app/<entity>/[id]/page.tsx`). Direct URL `/<entity>/<id>` рендерит Drawer поверх пустого `<main>`, закрытие → `router.push(listPath)`. URL клика из таблицы **не меняется** (e.preventDefault) — таблица остаётся видимой. Intercepted-routes Next.js 16 (`@modal/(..)<entity>/[id]`) нестабильны в dev и были отключены в фазе 7 после отладки. Детали — [`docs/plans/phase-7-drawers.md`](docs/plans/phase-7-drawers.md), [`docs/plans/phase-10-activities.md`](docs/plans/phase-10-activities.md) §0.
...
| 10 | Активности: timeline + Optimistic UI | F+B | `ActivityTimeline.tsx`, `ActivityForm.tsx`, `TaskCheckbox.tsx`, server action `toggleActivityDone` | Чекбокс задачи: мгновенный апдейт → откат + toast при ошибке; note/task визуально различимы; просроченные задачи подсвечены. **ID-формат (A15):** все server actions фазы 5–10 принимают и cuid, и slug-id (`seed-opp-*`); фикс в `src/lib/validators.ts` (regex `^[a-z0-9-]+$`). **Drawer рендерится через React Portal** в `document.body` — иначе `<div>` Drawer нельзя вложить в `<tr>` (hydration error). Используется `useState(serverDone)` + `useOptimistic(serverDone)` — state `serverDone` хранит подтверждённое значение с сервера; `useOptimistic` даёт мгновенный апдейт, откатывается при ошибке (useTransition завершается без revalidate). **RowWithDrawer-паттерн (фазы 7–10):** intercepted-routes Next.js 16 нестабильны в dev, поэтому таблицы используют client-side state (`<tr onClick={() => setOpen(true)}>` + `e.preventDefault()` на `<Link>`), Drawer через `<Drawer>` поверх таблицы. URL не меняется — таблица остаётся видимой. Direct URL `/<entity>/<id>` через `app/<entity>/[id]/page.tsx` + `CardOverlayWrapper` (для refresh/share-link) | прочитать **final-mvp.md** §7.4 + `Plan.md` §6.4 | [`docs/plans/phase-10-activities.md`](docs/plans/phase-10-activities.md) |
