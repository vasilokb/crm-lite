...
| 3. Модель данных Prisma (6 моделей + Stage) + миграция | ☐ | ☐ | `prisma migrate dev --name init-crm-schema` |
| 4. Seed контрольных данных (6/4/5/6/8 + ≥2 просроченных/сегодняшних задач) | ☐ | ☐ | детерминированный `npm run db:seed` (скрипт `tsx prisma/seed.ts`) |
...
| 3 | Модель данных Prisma + миграция | D | `prisma/schema.prisma`, `prisma/migrations/<ts>_init-crm-schema/migration.sql` | `npx prisma validate` — exit 0; `npx prisma migrate dev --name init-crm-schema` применён; 6 моделей (`Lead`, `Account`, `Contact`, `Stage`, `Opportunity`, `Activity`) + `leadId @unique` (D14) + `onDelete: Restrict` × 3 (Account→Contact, Account→Opportunity, Opportunity.activities→Activity) + `onDelete: SetNull` × 2 (Opportunity.stage, Lead.opportunity) | прочитай `prisma/schema.prisma` (если есть) + `Plan.md` §6.1 + **финал-mvp.md** §3.1 | [`docs/plans/phase-3-schema.md`](docs/plans/phase-3-schema.md) |
