...
| 5. Backend: server actions + Zod-валидация + CRUD | [x] | [x] | 2026-07-05 — tsc exit 0, smoke pass; db singleton + revalidate wrapper + 7 Zod-схем (Zod v3) + 5 server-action файлов (create/update/get для 4 сущностей + getActivities + updateOpportunityStage-stub); toggleActivityDone сигнатура D15; revalidatePath wrapped в safeRevalidate для совместимости со smoke вне Next runtime |
| 6. Frontend: списки + пагинация + поиск + фильтры | [x] | [x] | 2026-07-05 — tsc exit 0; 4 страницы (/leads, /accounts, /contacts, /opportunities) + /dashboard-stub + redirect /; 4 переиспользуемых компонента (Badge/Pagination/SearchInput/FilterBar); поиск `?q=`, фильтры `?source=` `?status=` `?stage=`; пагинация 50/page; Top-nav в layout; Next.js 16 async searchParams |
| 7. Drawer-карточки + связи + вложенная навигация | [x] | [x] | 2026-07-05 — Drawer overlay через parallel slot @modal + intercepting routes (.) (Next.js 16 не поддерживает (..) на root level); 4 intercepted + 4 full-страницы + Drawer/DrawerHeader/DrawerRelatedList + 4 Card + 4 Form (client); tsc exit 0; refresh на URL → full-page без overlay; refresh проверен на leads/accounts/opportunities/contacts. **2026-07-06 (7.2):** revert к IR (7.1 RowWithDrawer+CardOverlayWrapper удалены); E2E Playwright 15/15 pass (overlay, refresh, back, drawer-transition, optimistic-UI+IR). ⚠ TODO: обновить §6.7, §14 A9, phase-7-drawers.md под фактическую архитектуру (после фазы 11) |
| 8. Convert Lead: Accordion + `$transaction` + атомарность | [x] | [x] | 2026-07-05 — convertLead с $transaction (шаги a-f); P2002 catch → lead_already_converted; accountName обязателен (D12); prefill из Lead.company; race-test (Promise.all) pass: один ok + один error; UI: ConvertLeadAccordion внутри LeadCard → переиспользуется и в intercepted route, и в full-page; tsc exit 0 |
...
- `ActivityTimeline.tsx` (server, типы note/task визуально различимы: `border-l-4` color + label «✎ note» / «✓ task»; просроченные `border-rose-500`), `ActivityForm.tsx` (client, кнопки «+ note» / «+ task» + toggle-переключатель типа), `TaskCheckbox.tsx` (client, `useState(serverDone)` + `useOptimistic(serverDone)` + `useTransition`)
...
- **Drawer-модель** — **Next.js Intercepting Routes** (финальная архитектура, подтверждена E2E): root `layout.tsx` принимает parallel slot `@modal`; `src/app/@modal/(.)<entity>/[id]/page.tsx` перехватывает навигацию со списка → Drawer overlay через React Portal; `src/app/<entity>/[id]/page.tsx` — full-page для refresh/share-link. **URL обновляется** при клике из списка (не `e.preventDefault`). Список рендерит обычные `<tr>` + `<Link href="/<entity>/<id>">`. `Drawer` (client, Portal в `document.body`) по умолчанию закрывается через `router.back()`. **7.1 (RowWithDrawer + CardOverlayWrapper) откатаны в 7.2** — `useOptimistic` + `revalidatePath` совместимы с Intercepting Routes в Next.js 16.2.10 (Turbopack): Drawer НЕ закрывается после toggle task. Детали — [`docs/plans/phase-7-drawers.md`](docs/plans/phase-7-drawers.md), [`docs/plans/phase-10-activities.md`](docs/plans/phase-10-activities.md) §0.
...
- Реализация — **Next.js Intercepting Routes** (финальная, 7.2): root layout принимает parallel slot `@modal`; `src/app/@modal/(.)<entity>/[id]/page.tsx` перехватывает навигацию со списка (маркер `(.)` — same level, т.к. `@modal` и `<entity>` оба на корневом уровне `app/`); `src/app/<entity>/[id]/page.tsx` — «настоящая» страница для refresh/share-link. **Next.js 16.2.10 не поддерживает `(..)` на root level** — используется `(.)`.
...
- **E2E-верификация (Playwright, 2026-07-06):** 15/15 pass — overlay из списка, refresh → full-page, back закрывает, переход между Drawer'ами, **useOptimistic toggle task + revalidatePath НЕ закрывает Drawer**.
...
| 10 | Активности: timeline + Optimistic UI | F+B | `ActivityTimeline.tsx`, `ActivityForm.tsx`, `TaskCheckbox.tsx`, server action `toggleActivityDone` | Чекбокс задачи: мгновенный апдейт → откат + toast при ошибке; note/task визуально различимы; просроченные задачи подсвечены. **ID-формат (A15):** все server actions фазы 5–10 принимают и cuid, и slug-id (`seed-opp-*`); фикс в `src/lib/validators.ts` (regex `^[a-z0-9-]+$`). **Drawer через React Portal** в `document.body` (client, `mounted` gate для SSR). `useState(serverDone)` + `useOptimistic(serverDone)` + `useEffect`-синхронизация (M3 fix): state `serverDone` хранит подтверждённое значение с сервера и обновляется при revalidation; `useOptimistic` даёт мгновенный апдейт, откатывается при ошибке. **Архитектура (7.2 — Intercepting Routes):** `@modal/(.)<entity>/[id]` перехватывает клик из списка → Drawer overlay (URL меняется на `/<entity>/<id>`); `useOptimistic` + `revalidatePath` совместимы — Drawer НЕ закрывается после toggle task (E2E Playwright, 15/15). Direct URL `/<entity>/<id>` → full-page через `app/<entity>/[id]/page.tsx` (рендер `<XCard>` в `<main>`, без overlay). RowWithDrawer + CardOverlayWrapper (7.1) удалены. | прочитать **final-mvp.md** §7.4 + `Plan.md` §6.4 + §6.7 | [`docs/plans/phase-10-activities.md`](docs/plans/phase-10-activities.md) |
...
│   │   │   ├── default.tsx                   # null — Drawer не активен
...
| A5 | `React.useOptimistic` доступен в React 19 (Next.js 16) | Если версия React ниже 19 — fallback на ручной `useState` с rollback | Фаза 10.4 — добавить условие на версию React |
...
| A7 | Server actions в Next.js 16 достаточно для всего CRUD, включая Optimistic UI (через `useOptimistic` + `useTransition`); **отдельные `POST /api/...` endpoint'ы не нужны** | Если для revalidatePath понадобится API endpoint — добавить (не ожидается) | Фазы 5, 10 |
...
| A9 | **Drawer реализуется через Next.js Intercepting Routes**: `@modal` parallel route slot + `(.)<entity>/[id]` intercepting route (маркер `(.)`, не `(..)` — Next.js 16 не поддерживает `(..)` на root level) — overlay поверх списка, URL-driven, back закрывает Drawer. **Подтверждено E2E (Playwright, 2026-07-06):** useOptimistic + revalidatePath совместимы с IR в Next.js 16.2.10 (Turbopack) — Drawer не закрывается после toggle task. | ~~Если intercepting routes окажутся нестабильны~~ — 7.1 пробовала fallback (RowWithDrawer + CardOverlayWrapper), откатан в 7.2. | Фаза 7 → 7.2: структура `src/app/@modal/(.)<entity>/[id]/page.tsx`, root layout slot `{ modal }` |
...
| A13 | `Drawer` (фаза 7) **изначально спроектирован с `onCloseHref`** — `<Link href={onCloseHref}>` для backdrop/× в intercepted-route. Это работало для навигации **из списка** (history.back → список). Но для inline-форм (например, `CreateOpportunityForm` на `/opportunities`) `onCloseHref` уводил на предыдущую страницу в истории браузера (например `/contacts`). Решение: `Drawer` принимает `onClose?: () => void` callback; при отсутствии — fallback на `router.back()`. Для inline-форм передаём `onClose={() => setOpen(false)}`. | Если потребуется ещё один inline-сценарий (не из списка) — использовать тот же паттерн | Фазы 9, 10 — `Drawer` переработан (см. `src/components/Drawer.tsx`) |
| A14 | Кодировка файлов кириллицей: запись через `write`-инструмент/PS-`Out-File -Encoding utf8` (без BOM) **может сохранить кириллицу в сдвинутых UTF-8 байтах** (например `208,160` вместо `208,161` для `С`). Браузер при этом получает некорректный UTF-8 → отображает кракозябры (`РЎРІСЏР·Рё`). Признак: кириллица отсутствует в HTTP-ответе при декоде UTF-8, но есть в PowerShell-выводе как Latin-1. Решение: переписать файл через `write`-инструмент (он пишет валидный UTF-8). Проверка: `npx tsx -e "const fs=require(`"fs`"); const b=fs.readFileSync(`"file.tsx`"); const t=Buffer.from([0xD0,0xA1]).toString(`"utf-8`"); if (b.includes(Buffer.from([0xD0,0xA1,0xD0,0xB2]))) console.log(`"ok`");" | Команды `write`/`create_file` в разных IDE ведут себя по-разному | Фазы 7–10 — несколько файлов переписаны (см. `LeadCard.tsx`, `OpportunityCard.tsx`) |
| A15 | **ID-формат (Zod-валидация):** в `src/lib/validators.ts` helper `id = z.string().regex(/^[a-z0-9-]+$/).min(1).max(50)` принимает **и cuid, и slug-id** (`seed-opp-*`, `seed-lead-*`, …). Существование записи проверяет Prisma `findUnique`/`create`. **Обоснование:** seed (фаза 4) использует slug-id для детерминизма и читаемости, а `schema.prisma` сохраняет `@default(cuid())` для новых UI-записей. Без этого фикса `createActivity({opportunityId:"seed-opp-zeta-pavilion",...})` падает с `fieldErrors.opportunityId: ["Invalid cuid"]`. | Если в seed.slug появятся символы вне `[a-z0-9-]` — отклонится валидатором; добавить в regex | Фаза 10 — обнаружено и устранено (этот фикс). Затронуты все server actions фазы 5–10, использующие Zod-валидацию id: `createActivity`, `toggleActivityDone`, `updateOpportunityStage`, `createOpportunity`, `updateOpportunity`, `createContact`, `updateContact`, `createAccount`, `updateAccount`, `convertLead`. |
